...
class PCAE(nn.Module):
    ...
    def forward(self, x, device, mode='train'):
        ...
        multiplier = (std * math.sqrt(math.pi * 2)).reciprocal().unsqueeze(-1).mean(2).unsqueeze(-1)  # (B,1,1,1)
        power_multiply = (-(2 * (std ** 2))).reciprocal().unsqueeze(-1).mean(2).unsqueeze(-1)  # (B,1,1,1)
        ...
        template_detached = torch.cat(template_det, 0).unsqueeze(0).expand(x_m_detach.shape[0], -1, -1).to(
            device)  # (B,M,11*11)
        input_ocae = torch.cat([d_m_detach, x_m_detach, template_detached, c_z], -1)  # (B,M,144)

        return log_likelihood, input_ocae, x_m_detach, d_m_detach, c_z
...
class OCAE(nn.Module):
    ...
    def forward(self, inp, x_m, d_m, device, mode='train'):
        ...
        capsule_shape = (*object_parts.size()[:2], -1)
        object_capsule = torch.cat((ov_k.view(capsule_shape), c_k.view(capsule_shape), a_k.view(capsule_shape)), dim=2)
        temp_a = []
        temp_lambda = []
        ...
        identity = torch.eye(6).unsqueeze(0).unsqueeze(0).unsqueeze(0).expand(*diff.size()[:3], -1, -1).to(
            device)  # (B,K,M,6,6)
        ...
        gaussian_component = (a_k * a_kn) * (
            (a_k.sum(1).unsqueeze(1) * a_kn.sum(2).unsqueeze(1)).reciprocal())  # (B,K,M,1,1)
        ...
        return log_likelihood, a_k.squeeze(-1).squeeze(-1), a_kn.squeeze(-1).squeeze(-1), gaussian.squeeze(-1).squeeze(
            -1), object_capsule
